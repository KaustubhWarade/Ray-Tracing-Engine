...
HRESULT SceneOne::InitializeResources(void)
{
	...
	EXECUTE_AND_LOG_RETURN(resourceManager->LoadModel("KnightModel", "SceneOne/knightglb.glb"));
	m_pSceneModel = resourceManager->GetModel("KnightModel");
	if (!m_pSceneModel)
	{
		return E_FAIL;
	}
	Material modelMaterial;
	modelMaterial.Albedo = XMFLOAT3(0.8f, 0.8f, 0.8f);
	modelMaterial.Roughness = 0.2f;
	modelMaterial.Metallic = 0.8f;
	m_sceneData.Materials.push_back(modelMaterial);
	int modelMaterialIndex = (int)m_sceneData.Materials.size() - 1;

	for (const auto& mesh : m_pSceneModel->Meshes)
	{
